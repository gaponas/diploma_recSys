...
class NormalMeanG:
    ...
    def __init__(self, G: Callable, loc: float = 0, scale: float = 1):
        ...
        :param G: функция, которую применяем к нормальной величине и вычисляем среднее
        :param loc: среднее нормальной величины v
        :param scale: среднеквадратичное отклонение нормальной величины v
        """
        self.g = G
...
class G1:
    ...
    def __init__(self, a_const: float):
        """
        :param a_const: константа для вычисления функции, должна быть задана в диапазоне от 1 до 2
        """
        assert (1 <= a_const <= 2)
        self.a = a_const
    ...
    def __call__(self, u: torch.Tensor) -> torch.Tensor:
        ...
        :param u: выборка из некоторой случайной величины
        :return: результат применения функции к выборке
        """
...
class NegentropyApprox1:
    ...
    def __init__(self, a_const: float):
        """
        :param a_const: константа из функции G1.
        """
        self.g = G1(a_const)
    ...
    def __call__(self, x: torch.Tensor) -> float:
        ...
        :param x: выборка, для которой нужно вычислить негэнтропию
        :return: значение приближение негэнтропии для переданной выборки
        """
        g_x = self.g(x)
