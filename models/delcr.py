...
class DELCR(Module):
    ...
    def __init__(self, user_input_size: int, item_input_size: int, embedding_size: int, latent_factor_size: int,
        ...
        """
        :param user_input_size: размер входного вектора о пользователе
        :param item_input_size: размер входного вектора об элементе
        :param embedding_size: размер плотного вектора элементов, по которому выполняется начальное предсказание
        :param latent_factor_size: размеры внутренних слоев модели
        :param hidden_outputs: размер скрытого векторного представления сущностей
        """
...
class IndepDelsrLoss(Module):
    ...
    def __init__(self, negentropy_approx1: Callable, negentropy_approx2: Callable, lf_size: int, device: str = 'cpu',
                 indep_w: int = 1.0):
        """
        :param negentropy_approx1: функция для вычисления приближения негэнтропии,
        которую необходимо использовать при вычислении меры независимости компонент представлений пользователей
        :param negentropy_approx2: функция для вычисления приближения негэнтропии,
        которую необходимо использовать при вычислении меры независимости компонент представлений элементов
        :param lf_size: размер построенного моделью скрытого представления
        :param device: на чем выполняются вычисления
        :param indep_w: вес независимости в функции потерь
        """
...
class IndepSparseDelcrLoss(Module):
    ...
    def __init__(self, negentropy_approx1: Callable, negentropy_approx2: Callable, lf_size: int, device: str = 'cpu',
        ...
        :param device: на чем выполняются вычисления
        :param indep_w: вес независимости в функции потерь
        :param sparse_w: вес разреженности в функции потерь
        """
